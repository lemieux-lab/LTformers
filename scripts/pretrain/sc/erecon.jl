...
mc = (obj=:erecon, use_exp=use_exp, seq_len=seq_len, mask_ratio=config["mask_ratio"], mask_id=MASK_ID,
...
for epoch in ProgressBar(1:sched.n_epochs)
    ...
    for c in val_cache
        x_gpu, y_gpu = CuArray(c.x), CuArray(c.y)
        loss_val = use_exp ? masked_erecon_loss(model, x_gpu, y_gpu, CuArray(Float32.(c.m)))[1] :
                             masked_erecon_loss(model, x_gpu, y_gpu)[1]
        push!(val_eval_losses, Float32(loss_val))
    end
    ...
    if is_last
        ...
        for c in eval_cache
            x_gpu, y_gpu = CuArray(c.x), CuArray(c.y)
            loss_val, preds_masked, targets_masked =
                use_exp ? masked_erecon_loss(model, x_gpu, y_gpu, CuArray(Float32.(c.m))) :
                          masked_erecon_loss(model, x_gpu, y_gpu)
            push!(eval_losses, Float32(loss_val))
            (isnothing(preds_masked) || n_pred_batches >= max_pred_batches) && continue
            preds_cpu, targets_cpu = vec(cpu(preds_masked)), vec(cpu(targets_masked))
            append!(all_preds, preds_cpu)
            append!(all_trues, targets_cpu)
            accumulate_sq_errors!(err_acc, preds_cpu, targets_cpu, c.m, c.ids_or_ranks, hvg_idx)
            n_pred_batches += 1
        end
...
# plots + outputs
plot_loss(length(train_losses), train_losses, test_losses, save_dir, "MSE"; val_losses=val_losses)
cs, cp = if !isempty(all_preds)
    ...
    plot_idx = sample(1:length(all_trues), min(50000, length(all_trues)), replace=false)
