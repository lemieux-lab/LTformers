## mlm_inv.jl — inverse_ranks variant of PB MLM
## ETF predicts rank of gene (rank @ gene i) instead of gene ID at rank (gene @ rank i)
## Input expression stays in gene order (position i = gene i), not reindexed to rank order
## Labels are inverse_ranks: inv_ranks[gene, sample] = rank of that gene
...
if use_exp
    ...
    # gene-ordered expression: position i = gene i (NOT reindexed to rank order)
    X_train = X_expr[:, train_indices]
    X_test = X_expr[:, test_indices]
    # inverse permutation: inv_ranks[gene, sample] = rank of that gene
    # label at gene position i = rank of gene i
...
save_dir = joinpath("results", dataset_tag, "pretrain", "mlm_inv", config["modeltype"], timestamp)
mkpath(save_dir)
...
wandb = init_wandb(config, "PB-PT-Aug", "mlm_inv_$(fmt)_$(config["modeltype"])_$(timestamp)")
    ...
    for start_idx in 1:config["batch_size"]:size(X_test_masked, 2)
        ...
        if !isnothing(y_targets)
            ...
            for i in eachindex(y_targets_cpu)
                r = y_targets_cpu[i]  # r = rank (target class in inv_ranks mode)
            ...
            if is_last || done
                ...
                for j in 1:batch_len
                    for pos in 1:n_genes  # pos = gene position
