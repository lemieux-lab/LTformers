...
wandb = init_wandb(config, "PB-FT-Aug", "mlp_nopt_$(fmt)_$(config["level"])_$(timestamp)")
    ...
    if test_losses[end] < best_test_loss
        ...
        if is_regression
            ...
            log_params(config, gpu_info, 0, 0, best_dir;
                skip=mlp_skip, r2=best_r2, pearson=best_pearson, rmse=best_rmse,
                total_steps=global_step, best_epoch=best_epoch, best_test_loss=best_test_loss)
        else
            best_acc = mean(epoch_preds .== epoch_trues)
            log_params(config, gpu_info, 0, 0, best_dir;
                skip=mlp_skip, accuracy=best_acc, total_steps=global_step,
